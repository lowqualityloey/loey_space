// 06-Resources/scripts/src/lib/triage.ts
var DUMP_PATH = "00-Inbox/quick-capture-dump.md";
var ARCHIVE_SWEPT_LINES = true;
var TRIAGED_HEADING = "## \u2705 Triaged";
var ROUTES = {
  do: { kind: "task" },
  bin: { kind: "drop" },
...
var TOKEN_RE = new RegExp("#(" + Object.keys(ROUTES).join("|") + ")\\b", "i");
var TIME_CODE_RE = /`\s*\d{1,2}:\d{2}\s*(?:AM|PM)?\s*`/gi;
...
function deriveTitle(text, todayStr = getTodayStr()) {
...
function buildNote(route, title, text, noteCapturedDate, todayStr = getTodayStr()) {
...
function buildKanban(todayStr = getTodayStr()) {
  return [
    "---",
    "",
    "kanban-plugin: board",
